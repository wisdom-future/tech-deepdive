...
**Registry配置表 → RAW数据采集 → 专表信号识别 → 统一证据验证 → 多维度分析 → 线索整合 → 报告输出**
...
- **WF6**: 竞争线索 → Raw_Competitor_Intelligence
...
| N7-1-10 | Generator | SetVariables | 生成线索ID和相关标识 |
...
**触发方式：** 定时触发（每4小时）
**处理对象：** Tech_Intelligence_Master表中未验证的线索

| 节点 | Module类型 | Module名称 | 职责描述 |
|------|------------|------------|----------|
| N8-01 | Placeholder | Placeholder | 工作流起始点 |
| N8-02 | Data Storage | Google Sheets | 查询未验证线索记录 |
| N8-03 | Iterator | BasicFeeder | 迭代处理线索记录 |
...
| N8-12 | Data Storage | Google Sheets | 更新线索置信度字段 |
...
| N9-02 | Data Storage | Google Sheets | 查询已验证的线索数据 |
| N9-03 | Iterator | BasicFeeder | 迭代线索记录 |
...
| N9-12 | Data Storage | Google Sheets | 更新线索商业价值字段 |
...
| N12-10 | Data Storage | Google Sheets | 更新线索最终状态 |
...
| N13-02 | Data Storage | Google Sheets | 线索数据筛选排序 |
...
gantt
    ...
    WF7-6-竞争线索信号识别    :wf7-6, 04:30, 05:00
