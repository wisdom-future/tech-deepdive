...
function doGet(e) {
  ...
  return htmlTemplate.evaluate()
    .setTitle('联接技术洞察智能引擎')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.SAMEORIGIN) // Changed from ALLOWALL for security
...
function getPageContent(pageName) {
  try {
    ...
    logError(`获取页面内容失败 [${pageName}]: ${error.message}`);
...
function callApi(serviceName, methodName, args = []) {
  logDebug(`[ctrl.Code] callApi: ${serviceName}.${methodName} called.`);
  const services = {
  ...
  if (serviceName === 'debug' && methodName === 'ping') {
    return "Pong! Backend is alive at " + DateUtils.formatDate(new Date(), true);
  }
  ...
  if (services[serviceName] && typeof services[serviceName][methodName] === 'function') {
    try {
      ...
      if (result === null || typeof result === 'undefined') {
        logWarning(`[ctrl.Code] ${serviceName}.${methodName} returned null or undefined. Forcing an empty object {}.`);
      ...
      logError(`[ctrl.Code] Critical error executing ${serviceName}.${methodName}: ${e.message} \n ${e.stack}`);
      return { 
